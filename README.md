...
- **获客扩展（分层架构）**：公开落地页（landing）+ 分享菜单全页覆盖 + 成绩/天梯卡离屏绘制存相册 + 学校与考试深链 + 订阅引导 + 数据埋点；业务层 `services/` 与增长横切层 `growth/` 解耦
...
- **获客扩展（4 期）**：新增公开落地页（承载学校与考试参数，引导登录）；全页可分享到好友/朋友圈；成绩卡/天梯卡一键保存相册；登录后考试深链；订阅引导与数据埋点；业务层 `services/` 与增长层 `growth/` 完成分层解耦。

## 最近更新（2026-08-18）

> 本节汇总近期前端改动的用户可见部分，完整技术记录见 [CHANGELOG](./CHANGELOG.md)。

- **一键转发**：成绩报告与学科对比页新增「一键转发」按钮，自动将页面内容（含成绩数据、雷达图 / 差距柱状图、排版）绘制为高清海报，可分享到微信好友·朋友圈或保存到相册，并带「数据来自 X-exam 小程序」来源水印。长内容自动完整截取，不截断。
